...
int main()
{
	...
	for (;;) {
		printf("\033[2J"); 
		printf( "Alarm Counter: %d, flag %d\n", alarm_counter, change_flag);
		printf( "Lasted time: %lf, duration\n", duration);
		...
		if ( ADC_CHANNELS >= 1 )
		{
			...
			if (adc0 >= 0.0 ) {				
				adc_vol = adc0 * CONVERT_RATE;
			} else {
				adc_vol = 0.0;
			}
			
			printf("AIN0: %d in volts %3.2f\n" ,adc0, adc_vol);
			if ( ((adc_vol) >= 4.0) && change_flag == 0) {
				start_time = time(0);
				duration = 0.0;
				alarm_counter++;
				change_flag = 1;
